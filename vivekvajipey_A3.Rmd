...
bay_pums_heat_risk <-
  ...
  group_by(PUMA) %>% 
  summarize(
    perc_waterinsec_foodinsec_lowinc =
      sum(waterinsec_foodinsec_lowinc, na.rm =T)/sum(WGTP, na.rm = T)*100
  ) %>% 
  left_join(
    bay_pumas %>% 
      select(PUMACE10),
...
pums_pal <- colorNumeric(
  palette = "Reds",
  domain = bay_pums_heat_risk$perc_waterinsec_foodinsec_lowinc
)
...
leaflet() %>%
  ...
  addPolygons(
    ...
    fillColor = ~pums_pal(perc_waterinsec_foodinsec_lowinc),
    color = "white",
    opacity = 0.5,
    fillOpacity = 0.5,
    weight = 1,
    label = ~paste0(
      round(perc_waterinsec_foodinsec_lowinc), 
      "% with limited food and water access making less than $100K"
    ),
  ...
  addLegend(
    data = bay_pums_heat_risk,
    pal = pums_pal,
    values = ~perc_waterinsec_foodinsec_lowinc,
    title = "% with limited food and water access making less than $100K"
  )
```
